 ...
 *
 * Description: Plays the game of scramble
 *
 * Usage: ./scramble <optional seed>
 * 
 * 
 */
...
void scramble(void);
void binary(string s, int min, int max);
...
void scramble(void)
{
    ...
    {
        for (int j = 0; j < DIMENSION - 1 - i * 2; j++)
        {
            temp = grid[i][j + i];
            grid[i][j + i] = grid[DIMENSION - 1 - j - i][i];
            grid[DIMENSION - 1 - j - i][i] = grid[DIMENSION - 1 - i][DIMENSION - 1 - j - i];
            grid[DIMENSION - 1 - i][DIMENSION - 1 - j - i] = grid[j + i][DIMENSION - 1 - i];
            grid[j + i][DIMENSION - 1 - i] = temp;
        }
    }
}

// performs a binary search until the word is found, or no
...
void binary(string s, int min, int max)
{
    ...
    else
    {
        ...
        int stringCount;
        int sorter;
        if (strlen(s) > strlen(dictionary.words[middle].letters))
        {
            stringCount = strlen(dictionary.words[middle].letters);
        }
        else
        {
        ...
        {
            if (s[i] == dictionary.words[middle].letters[i])
            {
                sorter = 0;
            }
            else if (s[i] > dictionary.words[middle].letters[i])
            {
                sorter = -1;
            }
            else
        ...
        if (sorter < 0)
        {
            binary(s, middle + 1, max);
        }
        else if (sorter > 0)
        {
            binary(s, min, middle - 1);
        }
        else
        {
            returned = dictionary.words[middle].letters;
        }
    }
}